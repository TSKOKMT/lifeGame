...
void lifeGame::setup(int width_, int height_) {
    ...
    for (int y = 0; y < fbo.getHeight(); y += size) {
        ...
        bDestroys.push_back(vector<bool>());
        ...
        for (int x = 0; x < fbo.getWidth(); x += size) {
            ...
            bDestroys.back().push_back(false);
    ...
    for (int i = 0; i < cells.size(); i++) {
        sound.addFragment("/Users/tskokmt/Downloads/FRAME FREE/SOUND/D1.mp3", .15, true);
        sound.addFragment("/Users/tskokmt/Downloads/FRAME FREE/SOUND/D2.mp3", .15, true);
        sound.addFragment("/Users/tskokmt/Downloads/FRAME FREE/SOUND/D4.mp3", .15, true);
        sound.addFragment("/Users/tskokmt/Downloads/FRAME FREE/SOUND/D10.mp3", .15, true);
        sound.addFragment("/Users/tskokmt/Downloads/FRAME FREE/SOUND/D11.mp3", .15, true);
        sound.addFragment("/Users/tskokmt/Downloads/FRAME FREE/SOUND/D13.mp3", .15, true);
    }
    //sound2.addFragment("/Users/tskokmt/Downloads/FRAME FREE/SOUND/E17.mp3", .3);
    
    //sound.addLong("/Users/tskokmt/Downloads/FRAME FREE/SOUND/B0.mp3", .2, 60);
    
    sound3.addFragment("/Users/tskokmt/Downloads/FRAME FREE/SOUND/E9.mp3");
    
    soundPlayer.load("/Users/tskokmt/Downloads/FRAME FREE/SOUND/01/6.mp3");
    soundPlayer.setLoop(true);
    soundPlayer.play();
    
    soundPlayer2.load("/Users/tskokmt/Downloads/FRAME FREE/SOUND/01/1.mp3");
    soundPlayer2.setVolume(.1);
    soundPlayer2.setLoop(true);
    soundPlayer2.play();
    
    soundPlayer3.load("/Users/tskokmt/Downloads/FRAME FREE/SOUND/E17.mp3");
    soundPlayer3.setVolume(.2);
    soundPlayer3.setLoop(false);
    soundPlayer3.setMultiPlay(true);
    
    /*soundPlayer4.load("/Users/tskokmt/Downloads/FRAME FREE/SOUND/B0.mp3");
    soundPlayer4.setVolume(.5);
    soundPlayer4.setLoop(true);
    soundPlayer4.play();*/
}
...
void lifeGame::update() {
        ...
        for (int y = 0; y < cells.size(); y++) {
            for (int x = 0; x < cells[y].size(); x++) {
                if (cells[y][x] && !_cells[y][x] && !__cells[y][x]) {
                    ...
                    memoOfX = ofWrap(memoOfX, safeBox.getLeft(), safeBox.getRight());
                    memoOfY = ofWrap(memoOfY, safeBox.getTop(), safeBox.getBottom());
    ...
    safeBox.position = ofPoint(ofWrap(safeBox.position.x, 0, fbo.getWidth()), ofWrap(safeBox.position.y, 0, fbo.getHeight()));
    ...
    shootBox.position = ofPoint(ofWrap(shootBox.position.x, 0, fbo.getWidth()), ofWrap(shootBox.position.y, 0, fbo.getHeight()));
    _shootBox = shootBox;
    ...
    float ratio = math.easeFilter(sqrt(boundingBox.getArea()) / 2500., .01);
    //soundPlayer.setVolume(ratio);
    soundPlayer.setSpeed(ratio);
    
    if (!soundPlayer2.isPlaying()) soundPlayer2.play();
    ...
    for (int j = -1; j < (int)cells.size() + 1; j++) {
        int y = (j + cells.size()) % cells.size();
        for (int i = -1; i < (int)cells[y].size() + 1; i++) {
            int x = (i + cells[y].size()) % cells[y].size();
            if (cells[y][x]/* && !_cells[y][x] && !__cells[y][x]*/) {
                motions[y][x].begin(1, true, true, 0);
                bDestroys[y][x] = false;
            }
            if (cells[y][x] && !_cells[y][x] && !__cells[y][x]) {
                sound.playFragment();
            ...
            ofPoint addition = ofPoint(size * sin(ofGetElapsedTimef() * PI + x + y) * pow(1 - ratio, 2), -size * pow(1 - ratio, 3));
            if (!bDestroys[y][x]) ofDrawCircle((i + .5) * size + addition.x, (j + .5) * size + addition.y, size / 2. * ratio);
            else {
                ofSetCircleResolution(ofRandom(3, 7));
                ofPushMatrix();
                ofTranslate((i + .5) * size + addition.x, (j + .5) * size + addition.y);
                ofRotateDeg(ofRandom(360));
                ofDrawCircle(0, 0, size / 2. * ofMap(pow(ratio, 1), 1, 0, 1, 4 + (x % 4) + (y % 4)));
                ofPopMatrix();
...
//--------------------------------------------------------------
ofPoint lifeGame::getCellPosition(int xIndex_, int yIndex_) {
    ...
    do {
        ...
        p = ofPoint((xIndex_ + .5) * size, (yIndex_ + .5) * size);
        ofPushMatrix();
        translator.reset();
        translator.disSmartFill(ofRectangle(0, 0, width, height), shootBox);
        p = translator.getTranslatedPosition(p);
        ofPopMatrix();
        
        if (p.x < 0) {
            xIndex_ += cells.front().size();
            bOk = true;
        }
        if (p.y < 0) {
            yIndex_ += cells.size();
...
//--------------------------------------------------------------
void lifeGame::destroy(int xIndex_, int yIndex_) {
    
    if (cells[yIndex_][xIndex_]) {
        motions[yIndex_][xIndex_].begin(.1);
        cells[yIndex_][xIndex_] = false;
        bDestroys[yIndex_][xIndex_] = true;
        soundPlayer3.play();
    }
}
